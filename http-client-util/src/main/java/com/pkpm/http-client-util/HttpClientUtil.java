...
public class HttpClientUtil {
    ...
    /**
     * 查看资源链接情况，返回响应头header
     *
     * @param config 请求参数配置
     * @throws HttpProcessException http处理异常
     * @return 返回处理结果
     */
    public static Header[] respHeaders(HttpConfig config) throws HttpProcessException {
        return responseHeader(execute(config));
    }
    ...
    public static String mysend(HttpConfig config) throws HttpProcessException {
        return fmt2Json(execute(config), config.outenc());
    }
     ...
     * @return 返回处理结果
     */
    private static Header[] responseHeader(HttpResponse resp) throws HttpProcessException {
        Header[] respHeader;
        try {
            respHeader = resp.getAllHeaders();
            EntityUtils.consume(resp.getEntity());
        } catch (IOException e) {
            throw new HttpProcessException(e);
        } finally {
     ...
     * @return 返回处理结果
     */
    private static String fmt2Json(HttpResponse resp, String encoding) throws HttpProcessException {
        ...
        try {
            ...
            myHttpResponse.setBody(body);
            Header[] headers = resp.getAllHeaders();
            Map<String, Object> map = new HashMap<String, Object>();
            for (Header header : headers) {
                map.put(header.getName(), header.getValue());
            }
            myHttpResponse.setHeaders(map);
            myHttpResponse.setStatusCode(resp.getStatusLine().getStatusCode());
            EntityUtils.consume(resp.getEntity());

        } catch (IOException e) {
            myHttpResponse.setBody(e.getMessage());
            myHttpResponse.setStatusCode(resp.getStatusLine().getStatusCode());
            body = JsonUtil.serialize(myHttpResponse);
            throw new HttpProcessException(e);
        } finally {
            close(resp);
        }
        String response = JsonUtil.serialize(myHttpResponse);
        MyHttpResponse deserialize = JsonUtil.deserialize(response, MyHttpResponse.class);
	...
	/*public static void main(String[] args) {
		try {
			...
			Header[] headers= HttpHeader.custom().userAgent("X-Auth-Token").build();
			HttpClientUtil.create(httpConfig.url("https://iam.cn-north-1.myhuaweicloud.com/v3/auth/tokens"));
			//httpConfig.url("https://iam.cn-north-1.myhuaweicloud.com/v3/auth/tokens");
			System.out.println(httpConfig.json());

			String res = HttpClientUtil.post(httpConfig.headers(headers));
